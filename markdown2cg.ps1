param([String]$Source, [String]$ReleaseDestination, [String]$ReviewDestination, $Leagues = 0)
...
function export ($tree, $league, $subfolder) {
    ...
    if ($debug) {
        ...
        mkdir "$reviewDestination/$($subfolder)" -ErrorAction SilentlyContinue
...
function main() {
    $release = $null -ne $Destination
    $debug = $null -ne $reviewDestination
    if (-not (Test-Path $Source -PathType Leaf)) {
        Write-Error "the source:$source doesn't exist: create the file or check the path";
        exit 1;
    }
    elseif ( $null -ne $release -and $null -ne $debug) {
        Write-Warning "no actions required: define -Destination and/or -ReviewDestination ";
        exit 1;
    }
    ...
    # default league
    export -tree $tree -subfolder '' -league 256;

    if ($Leagues -gt 0) {
        1..($Leagues + 1) | % {
            export -tree $tree -subfolder "league$_" -league $_;
        }
    }       
}
